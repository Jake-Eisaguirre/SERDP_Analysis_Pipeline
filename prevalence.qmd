...
# Mean prevalence per site, per month, per species - Does having repeated data values affect model? Summarize or mutate
clean_prev <- raw_prev %>% 
...
# Mean prevalence per site, per year-month, per species
clean_prev <- raw_prev %>% 
  drop_na(species_capture) %>%
  drop_na(bd_swab_id) %>% 
  select(site, date, species_capture, bd_load) %>% 
  ...
  mutate(n_infected = sum(infected),
         ...
         prevalence = n_infected/n) %>% 
  ungroup() %>% 
  group_by(site, species_capture, year_month) %>% 
  summarise(average_prev = mean(prevalence)) %>% # summaries or mutate?
  ungroup()
...
for (i in 1:length(species_list)) {
  ...
  if (k > 8) {
      ...
      # add filter species list with k>8 - 4 species have too little data    
      spec_names[i] <- species_list[i]
      
      # filter clean data set for species list of k>8
      dat <- clean_prev %>% 
        select(species_capture, year_month, average_prev) %>% 
        filter(species_capture == species_list[i]) %>%  
        ungroup() %>% 
        select(!species_capture) %>% 
