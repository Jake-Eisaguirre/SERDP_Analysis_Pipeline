...
# not much general trend across time - Maybe slight negative trend through time
plot(clean_prev$month_year, clean_prev$average_prev)
...
for (i in 1:length(species_list)) {
  ...
  if (k > 8) {
      ...
      # run GAM per species
      gam <- gam(average_prev ~ s(unix), data = dat) 
      ...
      p <- plotGAM(gam, smooth.cov = 'unix') +
           theme_classic() +
           geom_point(data = dat, aes(x=unix, y = average_prev), alpha = 0.08, pch = 1) +
           ggtitle(paste(species_list[i]))
...
bull_dat <- clean_prev %>% 
  select(species_capture, average_prev, year_month) %>% 
...
bull_gam <- gam(average_prev ~ s(unix, k = 19), data = bull_dat, faimly=binomial(link = "logit"))
