...
librarian::shelf(tidyverse, RPostgres, DBI, here, lubridate, mgcv, voxel, scales, anytime, tidymv)
...
-   Am I properly capturing the varying raw population count per site per visit per species when using prevelance as a decimal? IE One day bull frogs at site x on day y had 3 out of 12 infected and then bull frogs at site x on day z had 7 out 28 infected. Is this a problem? offset(n)? 
`gam(n_infected ~ s(month, k = k) + offset(log(n)), data = dat, faimly = poisson(link = "log"))` <- this gives much nicer seasonal curves.
