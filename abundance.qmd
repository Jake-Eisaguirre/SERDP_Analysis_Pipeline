...
any time point through time what is the abundance at each site or in Pennn fit gam to VES counts
...
# 
# 
# mod <- gam(n ~ s(year_month, k = 117), data = bull)
# 
# plot(mod)
...
for (i in 1:length(species_list)) {
  ...
  if (k > 9) {
      ...
      p <- ggplot(newdata) + geom_line(aes(x=unix, y=pred)) +
                             ...
                             coord_cartesian(ylim = c(0, max(newdata$upper_se) + 5)) + ggtitle(species_list[i])s
