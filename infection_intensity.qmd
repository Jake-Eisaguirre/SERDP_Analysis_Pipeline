...
clean_int <- raw_int %>% 
  ...
  mutate(sample_bd_qty = if_else(is.na(sample_bd_qty), 0, sample_bd_qty),
         month = month(date),
         year = year(date)) %>% 
  rename(infection_intensity = sample_bd_qty)


plot(clean_int$date, clean_int$infection_intensity)
...
for (i in 1:length(species_list)) {
  ...
  if (k > 2) {
      ...
      dat <- clean_int %>% 
        select(species_capture, date, infection_intensity) %>%
        filter(species_capture == species_list[i]) %>%  
        mutate(month = month(date)) %>% 
        ungroup() %>% 
      ...
      # run GAM per species
      gam <- gam(infection_intensity ~ s(month, k = k), data = dat, faimly = binomial(link = "logit"))  
...
for (i in 1:length(species_list)) {
  ...
  if (k > 9) {
      ...
      # run GAM per species
      gam <- gam(infection_intensity ~ s(unix), data = dat, faimly = binomial(link = "logit")) 
